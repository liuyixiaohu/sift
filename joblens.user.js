...
(function () {
  ...
  const NO_SPONSOR_RE = new RegExp(
    NO_SPONSOR_KEYWORDS.map(k => k.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|"), "i"
  );
  ...
  const UNPAID_RE = new RegExp(
    UNPAID_KEYWORDS.map(k => k.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|"), "i"
  );
  ...
  // UI element refs (set during createUI)
  let ui = {};
  ...
  function getJobKey(card) {
    ...
    if (id) return "id:" + id;
    // fallback: title + company（极少情况下卡片无链接）
  ...
  function detailHasNoSponsorship() {
    ...
    for (const h of headings) {
      if (h.textContent.includes("About the job")) {
        ...
        if (text.length > 0) return NO_SPONSOR_RE.test(text);
      }
    }
    const article = document.querySelector("article");
    if (article) return NO_SPONSOR_RE.test(article.textContent);
    return false;
  }
  ...
  function detailHasUnpaid() {
    ...
    for (const h of headings) {
      if (h.textContent.includes("About the job")) {
        ...
        if (text.length > 0) return UNPAID_RE.test(text);
      }
    }
    const article = document.querySelector("article");
    if (article) return UNPAID_RE.test(article.textContent);
    return false;
  }
  ...
  function getDetailFingerprint() {
    ...
    for (const h of headings) {
      if (h.textContent.includes("About the job")) {
        const wrapper = h.parentElement;
        const contentSibling = wrapper?.nextElementSibling;
        if (contentSibling) return contentSibling.textContent.trim().substring(0, 200);
      }
    }
    return "";
  }
  ...
  function labelCard(card, reason) {
    ...
    for (const r of BORDER_PRIORITY) {
      if (existing.includes(r)) { card.dataset.ljFiltered = r; break; }
    }
  ...
  function applyBadges(card) {
    ...
    // 清除旧 badge（scope 元素和 visible 元素都清）
    card.querySelectorAll(".lj-badges").forEach(b => b.remove());
    if (target !== card) target.querySelectorAll(".lj-badges").forEach(b => b.remove());

    // 在可见元素上设置 border + position（inline style）
    target.style.position = "relative";
    target.style.overflow = "visible";
    let borderColor = "#D9797B";
    for (const r of BORDER_PRIORITY) {
      if (reasons.includes(r)) { borderColor = BADGE_COLORS[r]; break; }
    }
    ...
    reasons.forEach(reason => {
      ...
      badge.style.background = BADGE_COLORS[reason] || "#888";
  ...
  function refreshBadges() {
    ...
    if (labeledJobs.size > 0) {
      getJobCards().forEach(card => {
        ...
        for (const r of BORDER_PRIORITY) {
          if (arr.includes(r)) { card.dataset.ljFiltered = r; break; }
        }
        applyBadges(card);
  ...
  // ==================== 过滤列表中的卡片（检查所有条件，不 early return） ====================
  ...
  // ==================== 检查详情面板内容，标记到指定卡片 ====================
  // scan 路径直接传入 card（100% 准确），被动检测路径用 getActiveCard()
  ...
  // ==================== 点击卡片（多重策略） ====================
  // 优先级：div[role="button"] > 卡片链接 > 可见子元素 > card 本身
  // display:contents 元素无 layout box，直接 click() 可能无效
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      // Scan 按钮
      ".lj-scan-btn{background:#1F2328;color:#FAF7F2;border:none;border-radius:6px;padding:7px 0;cursor:pointer;font-weight:600;font-size:12px;font-family:'EB Garamond',Garamond,serif;width:100%;margin-top:12px;transition:opacity 0.2s}",
  ...
  function createUI() {
    ...
    const header = el("div", { className: "lj-header" }, [
      el("h3", { textContent: "JobLens v" + VERSION }),
      togBtn
    ]);
    ...
    header.addEventListener("mousedown", (e) => {
      if (e.target === togBtn) return; // toggle 按钮不参与拖动
    ...
    const companyInput = el("input", { type: "text", placeholder: "Company name (paste multi)..." });
    ...
    const titleInput = el("input", { type: "text", placeholder: "Keyword (paste multi)..." });
  ...
  function renderList(container, items, type) {
    ...
    if (items.length === 0) {
      container.appendChild(el("div", { className: "lj-empty", textContent: "None added yet" }));
      return;
    }
  ...
  function removeFromList(type, index) {
    ...
    document.querySelectorAll("[data-lj-reasons]").forEach((card) => {
      ...
      if (reasons.length === 0) {
        ...
        // 清除 scope 元素和 visible 元素上的 badge 和 inline style
        const target = getVisibleEl(card);
        card.querySelectorAll(".lj-badges").forEach(b => b.remove());
        if (target !== card) {
        ...
        for (const r of BORDER_PRIORITY) {
          if (reasons.includes(r)) { card.dataset.ljFiltered = r; break; }
        }
        applyBadges(card);
      }
  ...
  async function autoScanCards() {
    ...
    try {
      ...
      console.log("[LJF] Scan start: " + total + " cards to scan");
      updateScanButton("Scanning 0/" + total + "...");

      for (let i = 0; i < toScan.length; i++) {
        if (scanAbort) { console.log("[LJF] Scan aborted at " + i); break; }
        const card = toScan[i];
        if (card.dataset.ljReasons) continue;

        const title = getJobTitle(card);
        console.log("[LJF] Scan card " + (i + 1) + "/" + total + ": " + title);
        updateScanButton("Scanning " + (i + 1) + "/" + total + "...");
        ...
        // 直接用 card 引用检测，不经过 getActiveCard()（避免匹配错误）
    ...
    console.log("[LJF] Scan done, " + flagged + " flagged");
    updateScanButton("Scan Jobs");
    showToast("Scan done \u2014 " + flagged + " jobs flagged");
  }
  ...
  // ==================== 初始化 ====================
  function init() {
    console.log("[JobLens] v" + VERSION + " init at", location.pathname);
    createUI();
    filterJobCards();
    checkDetailPanel();
  }
  ...
  // ==================== 单一 Observer（合并 mutation 处理 + SPA 路由检测） ====================
  ...
  new MutationObserver(() => {
    ...
    if (location.href !== lastUrl) {
      ...
      if (location.href.includes("/jobs/")) {
        ...
        lastDetailText = "";
        setTimeout(() => {
          if (!document.getElementById("lj-filter-panel")) createUI();
          filterJobCards();
        }, 2000);
      }
    }
